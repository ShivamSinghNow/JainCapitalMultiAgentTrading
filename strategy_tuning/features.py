...
data = pd.read_feather('data/btc_1s_dev.feather')

# setting columns, timestamp, and index
data.columns = [
                'open_ts', 'open', 'high', 'low', 'close', 'volume', 'close_ts',
                'q_asset_volume', 'n_trades', 'taker_buy_b_volume', 'taker_buy_q_volume'
                ]

data = data[data['n_trades'] > 0]

data['timestamp'] = pd.to_datetime(data['open_ts'], unit='us')
data.drop(['open_ts', 'close_ts'], axis=1, inplace=True)
...
dx = np.where(denom == 0, 0, (np.abs(plus_di - minus_di) / denom) * 100)
data['adx'] = pd.Series(dx, index=data.index).rolling(LOOKBACK).mean()
